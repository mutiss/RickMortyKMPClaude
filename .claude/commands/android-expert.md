...
Once the build passes and all files are confirmed clean, **immediately** invoke the `pr-manager` skill with the same `$ARGUMENTS` — do not wait for the user to type it. The pipeline continues automatically.
